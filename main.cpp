...
// 寻找float* 的最大值的索引
int find_max_index(float* data, int size)
{
	float max_value = 0;
	int max_index = 0;
...
void print_data(float* data, int size)
{
	...
	for (int i = 0; i < size; i++)
		std::cout << data[i] << " ";
	std::cout << std::endl;
	std::cout << "目前最大值: " << find_max_index(data, size) << std::endl;
	std::cout << std::endl;
...
void inference_model(Ort::Session& session, std::vector<std::string>& input_names, std::vector<std::vector<float>>& input_tensor_values_list, std::vector<std::int64_t>& input_shapes, std::vector<std::string>& output_names, std::vector<short>& t_data, std::vector<std::vector<std::vector<std::vector<mmwave_handle::Complex>>>>& data_radar, int& cnt)
{
	...
	while (1)
	{
		...
		if (!data_queue.empty())
		{
			...
			float maxx_value[class_cnt] = { 0 };

			last_result = -1;
			last_result_value = 0;

			// 起始时间(一共4个时间窗口, 每个时间窗口30帧, 每帧64个点, 对每一帧进行推理)
			for (int t = 20; t <= 30; t += 5)
			{
				int cnt = 0;
				...
				//print_data(output_data, class_cnt);

				// 当前推理结果
				int current_result = find_max_index(output_data, class_cnt);
				float current_result_value = output_data[current_result];

				//if ((last_result == current_result) && (current_result != 4) && (last_result_value + current_result_value >= 1.5)) {
				//	std::cout << "连续两次推理结果相同,结果为: " << current_result << std::endl;
				//	std::cout << (last_result_value + current_result_value) << std::endl;
				//	// 绘制特征图
				//	plt::clf();
				//	mmwave_handle::plot_features(input_tensor_values_list_local);
				//	plt::pause(0.01);
				...
				last_result_value = current_result_value;

			}

			//
			max_index = find_max_index(maxx_value, class_cnt);
			max_value = maxx_value[max_index];
			if (max_index != 4&&max_value>1.2) {
				plt::clf();
				mmwave_handle::plot_features(input_tensor_values_list_local);
				plt::pause(0.01);
				print_data(maxx_value, class_cnt);
			}
			//print_data(output_data, class_cnt);
			// 更新上一个数据
